...
19. **治本演示**：修复源系统的输入校验 → 重新同步 → 该缺陷不再出现 →
    对应清洗规则标记为可退役。**活跃规则数第一次下降**
20. 展示 Phoenix trace 时间线：完整工具链路、耗时、成本，其中 `WAITING_FOR_HUMAN` 一行直观呈现挂起与恢复
21. 展示 Eval 结果：N 个注入错误的检出率、误修率、Unsafe Write Rate = 0.0%

**核心演示点**：第 6 步（负载护栏）、第 10 步（转发批不动）、第 11 步（WIP 触顶转向）、第 13 步（权限遮蔽）、第 14–15 步（超时升级与优雅放弃）、第 16 步（harness 可恢复）、**第 19 步（治本：清洗规则退役）**。
