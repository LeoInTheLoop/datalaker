...
回归入口：`HERMES=<hermes-agent 路径> ./tests/run_all.sh` → **94 条断言全绿**
...
| 15 | EXPLAIN 预估、串行队列、时间窗口、熔断 | 8.1 | 🟡 | 接口在，护栏逻辑待补 → R3 |
| 16 | AST 准入（替换关键字判断） | 8.1 | 🟡 | 占位实现，`TODO(R3)` 换 sqlglot |
| 17 | 四种接入方式（SQL / CSV / Excel / 邮件附件） | 16.4 | ❌ | R2 |
| 18 | 增量同步与新鲜度 | 6.1 | ❌ | R2 |
| 19 | **平台安全基线（TLS / 认证 / 授权）** | 15 R3 | ❌ | **R3——卡住 R5** |
| 20 | 角色化审批、双重确认、回执 | 10.4–10.6 | ❌ | R3 |
| 21 | IMAP 收信、意图分类、入站路由 | 10.1–10.3 | ❌ | R4 |
| 22 | WIP 限制 | 10.7 | ❌ | R4 |
| 23 | 停止点、DQ 门槛、3 轮上限、周报 | 5.2、5.3、5.5 | ❌ | R4 |
| 24 | Remediation Ledger 与整改闭环 | 7、7.1 | ❌ | R5 |
| 25 | 权限治理与 Policy Sync | 11 | ❌ | R5 |
| 26 | Dirty Data Injector 与 Eval | 16 | ❌ | R5.5 |
| 27 | Phoenix 可观测 | 13 | ❌ | R1 剩余 |
| 28 | 媒体线（whisper + 分类） | 12 | ❌ | R6，可裁剪 |
