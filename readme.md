...
| 问 | workflow（代码） | agent（模型） |
|---|---|---|
| 同样输入是否要求同样输出？ | **是** | 否，要「看情况」 |
| 决策点 | 无，或可写成规则 | 需要推理与权衡 |
| 输入结构 | 固定 | 多变 / 半结构化 |
| 失败模式 | 可枚举，可测 | 不可预期 |
| 成本 | 近零 | 每步 token |
...
| 纯 workflow（不进模型） | 交给模型判断 |
|---|---|
| 单表 profiling（空值率、格式分布、值域） | 「这算不算问题」 |
| CSV / Excel 解析与类型推断 | 多表关系推断、口径歧义识别 |
| Schema 比对与漂移检测 | 清洗方案提案 |
| 增量同步执行、分区裁剪 | 找谁问、问什么（5.7） |
| 账本写入、周报数据聚合 | 邮件意图分类（10.2） |
| 令牌签发校验、权限判定 | 权限收敛提案（11.5） |

用户提到的几个场景正好落在两侧：

- **单个表格 / PDF 存素材库、单表数据清洗** → workflow。结构固定、规则明确、要求可重复
- **多个表格联动** → 交给模型。关系推断需要业务语义，规则写不全
...
```
workflow  扫描 orders.customer_id
          → 50 个 NULL，占 0.05%，这 50 条 payment_type 全为 voucher
                              ↓  只把这一行结论送进上下文
模型      判断：像是匿名订单而非数据错误 → 生成提问（5.7）
```
...
#### 支柱四：执行形态 —— 固定 pipeline 还是模型在线 ❌

4.5 定了判据（**能用代码做的，不问模型**），但**没有实现分层**：
目前所有事情都是 Agent 调工具，没有「这条路径直接走代码、不进模型」的机制。

真正要区分的是**这段工作需不需要模型在线**：

| | Pipeline 模式 | Agent 模式 |
|---|---|---|
| 需要 LLM 在线 | ❌ | ✅ |
| 增量同步、profiling、账本聚合、周报生成 | ✅ | |
| 判断脏数据、生成提案、决定找谁问 | | ✅ |

**这带来一个可用性收益**：模型额度耗尽、网络中断、端点故障时，
Pipeline 部分照常运行——数据继续同步、账本继续记、周报继续发，
只是暂时不做判断。**降级，而不是停摆。**

（对应 5.6 的预算兜底：超预算时 L0 仍放行，正是这个思路的局部体现。）

R2 要建立的分层：

```
纯函数（services/，不注册为工具）
  单表 profiling、CSV/Excel 解析、schema 比对、增量执行、账本聚合
        ↓ 只把结论交给
工具（policy.py 注册，受门禁约束）
  ingest_table、apply_cleaning_rule、publish_gold …
        ↓ 由
Agent loop
  判断、提案、找谁问
```

> **工具注册表里每多一个工具，模型就多一条可能走错的路。**
> 能写成纯函数的就不要注册为工具——这既是省 token，也是缩小攻击面。

判断法则：**同样输入要求同样输出 → 纯函数；需要「看情况」→ 工具或 Agent。**
...
| 回执 | 10.6 | ✅ |
| **数据工具实现（支柱二）** | **4.6** | ⬜ |
| **执行形态分层：Pipeline / 工具 / Agent（支柱四）** | **4.5、4.6** | ⬜ |
