...
**已经是这个形状**：数据通道禁 join，元数据通道允许在 `pg_catalog` 上 join。
...
| 护栏 | 措施 |
|---|---|
| **禁止关系展开** | 「源上永不 join」的 API 版：SOQL 关系查询与子查询、HubSpot `associations`、Stripe/Notion `expand` 一律拒。一个 Contact 展开 Account 是 N+1 次隐式取数，**而请求看起来只有一个** |
...
| **R3** | 入站交互（IMAP 收信 / header 归属 / 意图分类 / 双重确认）+ 平台安全基线（TLS→认证→授权）+ AST 准入 + 数据源宽度（含 **SaaS 导出源**）+ 增量同步 | ⬜ | 匿名连不上 Trino；五种接入结果等价；SaaS 定时报表自动落 bronze；权限元数据 dump 出得来；转发的链接批不动 |
...
| 17 | 五种接入方式（SQL / CSV / Excel / 邮件附件 / SaaS API） | 16.4 | ❌ | R3 |
| 17b | SaaS 数据面：定时报表 → 邮件附件路径 | 8.2 | ❌ | R3 |
| 17c | **SaaS 控制面：权限元数据只读 dump** | 8.2、11.2 | ❌ | **R3 取数 → R4 出结论** |
