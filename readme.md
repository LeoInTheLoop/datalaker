...
| **Runtime** | 什么走 agent loop，什么走 workflow | ❌ **只有设计** | **R2** |
...
> 若设为「需审批」，则每个内置工具都发一封邮件（噪音，人会开始无脑点批准），
...
同时要做减法：**Hermes 侧配置 `disabled_toolsets`**，
把不需要的内置工具在框架层关掉，而不是只靠我们的 gate 拦（双保险）。
...
1. **联系人记忆** —— 谁管哪个域、谁回信快、谁已离职。
   现在「该找谁」每次都要重新问 Sponsor。→ **R2**
2. **语义检索** —— `known(asset, key)` 只能精确匹配。
   问「客户表的口径」找不到已沉淀的 `customers.name` 说明。→ R4

> 记忆的价值不在存，在**取回时机**：在 Agent 准备提问之前先查一次，
> 命中就不问。否则沉淀了也等于没有。

#### 支柱四：Runtime —— loop 还是 workflow ❌
...
| 回执 | 10.6 | ✅ |
| **数据工具实现（支柱二）** | **4.6** | ⬜ |
| **Runtime 分层：纯函数 / 工具 / loop（支柱四）** | **4.5、4.6** | ⬜ |
| **联系人记忆（支柱三）** | **4.6** | ⬜ |
| **Hermes 侧 `disabled_toolsets`（双保险）** | **4.6** | ⬜ |
| 停止点判定、阶段成果交付 | 5.2 | ⬜ |
| DQ 门槛量化、3 轮修复上限 | 5.3 | ⬜ |
