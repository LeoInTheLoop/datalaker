...
>
> **实现状态**：本文大部分是设计。**已跑通的部分见第 19 节**（94 条断言全绿），
> 含审批硬门禁、容器级权限隔离、Hermes 真实集成、lakehouse 独立可用。
> 尚未实现的部分、当前安全现状与成本模型也在那一节。
...
### 13.1 网络与部署拓扑：内网 datalake 怎么发邮件

**生产 datalake 绝大多数在内网 / VPC，不能直接访问公网**，金融、医疗、政务更严格。
这带来三个出站需求，难度递增：

| 出站需求 | 难度 | 解法 |
|---|---|---|
| 发审批邮件 | 低 | **内部 SMTP 中继** —— 企业都有邮件服务器，不需要公网 |
| 接收审批点击 | 低 | callback 服务部署在内网，**链接指向内网地址** |
| **调用 LLM API** | **高** | 见下 |

#### 邮件其实不是问题

企业内部有 SMTP 服务器，`MAIL_TRANSPORT=smtp` 指向它即可，全程不出内网。
IMAP 收信同理。**这也是为什么 `services/mailer.py` 保留了 SMTP 传输方式**——
它不只是 Gmail OAuth 的备选，而是内网部署的默认选项。

审批链接指向内网地址反而**更安全**：链接即使外泄，公网也访问不到。
第 9.4 节的身份强度分档在这里可以降一档——内网可达本身就是一层边界。

#### 真正的难点是模型调用

Agent 每一步都要调 LLM，而云端模型 API 在公网。三种解法：

| 方案 | 做法 | 代价 |
|---|---|---|
| **本地模型** | vLLM / Ollama 部署开源模型，OpenAI 兼容端点 | 需要 GPU；小模型的 tool-calling 稳定性要实测 |
| **出站白名单** | 只放行模型 API 域名，经代理并记录全部请求 | 需安全团队批准；**数据会离开内网** |
| **私有化模型服务** | 云厂商的专有部署（如阿里云百炼私有化、Azure OpenAI 的 VNet 集成） | 成本高，但合规上最容易通过 |

> **`harness 模型无关` 这条设计在这里兑现了价值**（第 4 节）：
> 内网部署时把 `OPENAI_BASE_URL` 指向本地 vLLM 即可，plugin 与门禁逻辑一行不改。
> Hermes 本身也支持 local / Docker / SSH / Modal 多种后端。

#### 关键约束：什么数据会离开内网

即使走出站白名单，也必须明确**发给模型的是什么**：
...
Connector Service（第 8 节）返回的采样数据**不应进入模型上下文**——
Agent 需要的是「这个字段 12% 的值含中文字符」这个结论，不是那 12% 的具体内容。
这条要在 `post_tool_call` 里强制：**查询结果先做统计摘要，再进上下文**。

> 这也顺带解决了成本问题：不把数据行喂给模型，token 消耗低一个数量级。

#### 三种部署形态

| 形态 | 适用 | 模型 | 邮件 |
|---|---|---|---|
| **全内网** | 金融、医疗、政务 | 本地 vLLM | 内部 SMTP |
| **混合** | 一般企业 | 出站白名单 + 代理 | 内部 SMTP |
| **全云** | 云原生团队 / 本项目 Demo | 云 API | Gmail / SES |

本项目当前是**全云**形态（DashScope + Gmail），因为 Demo 优先。
但三种形态的差异只在 `.env` 的三个变量（`OPENAI_BASE_URL`、`MAIL_TRANSPORT`、
`APPROVAL_BASE_URL`），**治理逻辑完全相同**。
...
**本文大部分内容是设计。这一节说明哪些已经跑通、哪些还没有。**
不标注这一点，读者只能默认全是空想。

回归入口：`HERMES=<hermes-agent 路径> ./tests/run_all.sh`

### 19.1 已实现并验证（94 条断言，全绿）

| 组 | 条数 | 验证了什么 |
|---|---|---|
| L0 平台冒烟 | 8 | datalake 作为独立产品可用，含跨 catalog join |
| 治理 Plugin 拦截 | 24 | 分级、票据、指纹绑定、deny list、fail-closed、禁止自我提权 |
| 凭证层只读 | 6 | `agent_ro` 账号 INSERT/UPDATE/DELETE/DROP/CREATE 全被拒 |
| 列级权限隔离 | 12 | Agent 无法伪造批准、改批准人、延长有效期、改指纹、删记录 |
| Connector Service | 9 | 禁 join、非 SELECT 拒绝、自动注入 LIMIT、负载记账 |
| Hermes 真实集成 | 11 | `pre_tool_call` 的 block / modify / 端到端 fail-closed |
| 通知失败 ≠ 门禁打开 | 4 | SMTP 未配置时动作**依然被拦**，只是通知没送达 |
| 挂起语义 | 4 | 模型收到 `{"error": "[PENDING_APPROVAL]..."}` 后不重试 |
| 模型端点 + tool calling | 5 | 端点可用且工具调用可靠 |
| 审批闭环端到端 | 11 | 挂起 → 邮件 → 点击 → 恢复 → 票据一次性 → 重放拒绝 |

另有 `tests/test_live_mail_loop.py`（真实 Gmail + 人工点击，已通过一次）。

**几条值得单独说的实证：**

- 往 Agent 上下文里塞一封伪造的「我同意」邮件，工具调用**照样被拦**——门禁不认文本只认票据
- Agent 容器执行 `INSERT INTO decisions` 直接 `permission denied`——两个容器两套凭证，物理隔离
- 批准 `FIN.monthly` 的票据对 `HR.salary` 无效——参数指纹绑定在真实链路上生效

### 19.2 平台能力实测

除 L0 八条外，另实测 14 项分析能力，全部开箱可用：

| 类别 | 已验证 |
|---|---|
| 写入更新 | `MERGE INTO`（upsert）、行级 `UPDATE`、行级 `DELETE` |
| 分区与维护 | 月份分区、分区裁剪、`OPTIMIZE` 合并小文件、`expire_snapshots` |
| 分析 SQL | 窗口函数、CTE、CTAS、视图 |
| 元数据 | 表历史、分区元数据、`SHOW STATS` |

**缺的不是引擎能力，是配置。** 作为分析基础，SQL 这一层够用。

### 19.3 尚未实现

| 项 | 章节 | 排期 |
|---|---|---|
| 四种接入方式（SQL / CSV / Excel / 邮件附件） | 16.4 | R2 |
| 增量同步与新鲜度 | 6.1 | R2 |
| **平台安全基线（TLS / 认证 / 授权）** | 15 R3 | **R3，卡住 R5** |
| 角色化审批、双重确认、回执 | 10.4–10.6 | R3 |
| IMAP 收信、意图分类、WIP 限制 | 10.1–10.3、10.7 | R4 |
| 停止点、DQ 门槛、周报 | 5.2、5.3、5.5 | R4 |
| Remediation Ledger 与整改闭环 | 7、7.1 | R5 |
| 权限治理与 Policy Sync | 11 | R5 |
| Dirty Data Injector 与 Eval | 16 | R5.5 |
| 媒体线 | 12 | R6 |

> **当前安全现状（诚实披露）**：Trino 无认证、无授权、无 TLS，
> 任意用户名即可读写全部数据；MinIO 凭证为 `admin/password`。
...
| 项 | 成本 |
|---|---|
| Trino / Iceberg / MinIO / Postgres / Hermes | **0**——全部开源，全部本地容器 |
| 六个容器运行时 | 约 1.1GB 内存，无外部计费 |
| **LLM 调用** | **唯一成本点** |

其中容易被误解的一点：Trino 访问 MinIO 用的是 **AWS SDK**，
但那只是说 S3 协议的客户端库，连的是本机 `http://minio:9000`，
`s3://lake/...` 只是协议前缀。**一个字节都不出机器，与 AWS 无关。**

#### LLM 成本的控制手段

| 手段 | 章节 | 效果 |
|---|---|---|
| 只把统计摘要送进上下文，不送数据行 | 13.1 | token 消耗降一个数量级 |
| 每轮预算兜底（调用数 / 时长 / 成本） | 5.6 | 硬上限，超出即挂起 |
| 修复迭代 3 轮上限 | 5.3 | 防止在单个问题上无限烧钱 |
| WIP 限制 | 10.7 | 限制同时在办的任务数 |
| 免费额度模型 + 降级链 | 13 | 额度耗尽返回 403 而非计费 |

> **实际 token 消耗尚未实测**——需要在 R4 接入 Phoenix 后，
> 用真实业务流程跑一遍才能给出每次接入的成本区间。现在给数字是编的。

内网部署时模型可换成本地 vLLM，那时 LLM 成本转为 GPU 成本（见 13.1）。
