...
| **R2** | 数据源宽度：SQL（账号密码）/ CSV / Excel / **邮件附件** + 凭证管理 + 资产登记；**增量同步与新鲜度（6.1）**；L0 平台冒烟（16.0） | 四种接入方式结果等价；**第二次同步能拿到新增与变更**；datalake 可独立交付 |
...
| 17 | 四种接入方式（SQL / CSV / Excel / 邮件附件） | 16.4 | ❌ | R2 |
| 20 | 增量同步与新鲜度 | 6.1 | ❌ | R2 |
| 21 | **平台安全基线（TLS / 认证 / 授权）** | 15 R3 | ❌ | **R3——卡住 R5** |
| 22 | 角色化审批、双重确认、回执 | 10.4–10.6 | ❌ | R3 |
| 23 | IMAP 收信、意图分类、入站路由 | 10.1–10.3 | ❌ | R4 |
| 24 | WIP 限制 | 10.7 | ❌ | R4 |
| 25 | 停止点、DQ 门槛、3 轮上限、周报 | 5.2、5.3、5.5 | ❌ | R4 |
| 26 | Remediation Ledger 与整改闭环 | 7、7.1 | ❌ | R5 |
| 27 | 权限治理与 Policy Sync | 11 | ❌ | R5 |
| 28 | Dirty Data Injector 与 Eval | 16 | ❌ | R5.5 |
| 29 | Phoenix 可观测 | 13 | ❌ | R1 剩余 |
