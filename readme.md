...
| 19 | **通知通道可替换（邮件 / 飞书 / 企微）** | 13.2 | ✅ | 三通道可实例化；邮件已实测发信 |
| 20 | 回执（批准后告知批了什么） | 10.6 | 🟡 | callback 已接入，待真实通道验证 |
| 21 | Cloudflare Tunnel 编排 | 13.2 | 🟡 | compose 已就绪，待填 token |
| 16 | EXPLAIN 预估、串行队列、熔断 | 8.1 | 🟡 | 接口在，护栏逻辑待补 → R3 |
| 22 | AST 准入（替换关键字判断） | 8.1 | 🟡 | 占位实现，`TODO(R3)` 换 sqlglot |
| 17 | 四种接入方式（SQL / CSV / Excel / 邮件附件） | 16.4 | ❌ | R3 |
| 23 | 增量同步与新鲜度 | 6.1 | ❌ | R3 |
| 24 | **平台安全基线（TLS / 认证 / 授权）** | 15 R3 | ❌ | **R3——卡住 R4 的权限治理** |
| 25 | 角色化审批、双重确认、回执 | 10.4–10.6 | ❌ | **R2** |
| 26 | IMAP 收信、意图分类、入站路由 | 10.1–10.3 | ❌ | **R2** |
| 27 | WIP 限制 | 10.7 | ❌ | **R2** |
| 28 | 停止点、DQ 门槛、3 轮上限、周报 | 5.2、5.3、5.5 | ❌ | **R2** |
| 29 | Remediation Ledger 与整改闭环 | 7、7.1 | ❌ | R4 |
| 30 | 权限治理与 Policy Sync | 11 | ❌ | R4 |
| 31 | Dirty Data Injector 与 Eval | 16 | ❌ | R5 |
| 32 | Phoenix 可观测 | 13 | ❌ | R1.5 |
| 33 | 媒体线（whisper + 分类） | 12 | ❌ | R6，可裁剪 |
