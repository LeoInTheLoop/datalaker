...
### 8.2 SaaS / API 源：护栏要重写，不能照搬

20–50 人公司的数据大头**不在数据库里**。CRM 里的客户与商机、在线表格里的排期、
财务 SaaS 里的流水——这些没有 DSN，只有 REST API 和 OAuth token。
它们必须进 lake，但 8.1 的整套护栏建立在两个 SQL 世界的前提上，换成 API 后**两个都不成立**：

| SQL 世界的前提 | SaaS 世界 |
|---|---|
| 代价可预估（`EXPLAIN` 出扫描行数） | **翻到最后一页才知道有多少** |
| 代价由源系统自己承担（慢查询） | 代价是**共享配额**，用完了公司**生产集成一起挂** |

第二条是关键差别，也是 8.1 里完全没有对应物的一类损伤：
拖慢生产库最多是慢，**耗尽 Salesforce 当日 API 配额会让财务的对账同步、
客服的工单回写在同一天一起失败**——而且失败的不是 Claw，是别人的活。

#### 对象白名单 + 禁止关系展开

「源系统上永不 join」的 API 对应物是**永不展开关联对象**。主流平台都提供这个能力，都要挡：

| 平台 | 关联展开语法 | 处置 |
|---|---|---|
| Salesforce | SOQL 关系查询 `SELECT Account.Name FROM Contact`；子查询 `(SELECT … FROM Contacts)` | 拒绝：字段名含 `.`，或存在嵌套 `SELECT` |
| HubSpot | `?associations=companies,deals` | 拒绝：`associations` 参数一律剥离 |
| Stripe / Notion | `expand[]=customer` | 拒绝：`expand` 一律剥离 |

理由与 8.1 逐字相同：**关联的代价不可预估**。一个 Contact 展开 Account 是 N+1 次隐式取数，
配额消耗是纯列表页的几十倍，**而请求看起来只有一个**——比 SQL 的笛卡尔积更隐蔽。

SOQL **不能复用 `_admit()`**：它不是 SQL 方言，`sqlglot` 会把它解析成似是而非的树，
按那棵树做判断等于没判断。写独立的 SOQL 准入，且用**白名单文法**而非黑名单正则——
只接受 `SELECT <裸字段列表> FROM <单对象> [WHERE …] [ORDER BY Id] [LIMIT n]`，
不在文法内的一律拒，包括看起来无害的写法。

> 白名单文法的代价是新写法要手工放行。这个代价对 20–50 人公司完全可接受——
> 他们的 CRM 自定义对象数量是两位数，不是两千。

#### 配额必须先预留，再调用

`EXPLAIN` 在这里没有对应物，因此改用**预留制**：Connector Service 为每个源维护当日配额账本，
调用前按「预估页数 × 每页 1 次」先扣减，扣不动就拒绝并排队到次日。

| 项 | 措施 |
|---|---|
| 预算 | 每源每日调用上限，**默认取该源官方配额的 20%**，其余留给对方既有的生产集成 |
| 预留 | 分页任务开始前一次性扣减预估页数；中途不足则中断并保留水位线，下次续跑 |
| 熔断 | 连续 429 / `REQUEST_LIMIT_EXCEEDED` 立即暂停该源全部任务并通知 Owner |
| 记账 | 并入 8.1 的负载记账：周报里写「本周消耗贵司 Salesforce 配额 3.2%」 |

**20% 这个数字要写进接入审批邮件让 Owner 确认，不能自己定**——
他知道生产集成有多吃紧，我们不知道。

#### 分页读不是快照

SQL 的一次 `SELECT` 是一致性快照，**翻十分钟的分页不是**。
按 `updated_at` 排序时，翻页过程中被修改的记录会跳到后面的页——**同时造成重复和漏读**。

处置：一律按不可变主键排序（`ORDER BY Id`），增量条件用 `SystemModstamp >= 水位线`，
水位线**回退一个安全窗口**（默认 5 分钟）容忍时钟偏差，产生的重复由 bronze 的幂等 upsert 吸收。

#### 删除是看不见的

`updated_at` 增量同步**看不到删除**：记录消失了，水位线上什么都不会出现。
这在 SQL 源上同样存在，但 CRM 里删除是日常操作——重复线索合并、无效客户清理——量级完全不同。

| 平台 | 可用手段 |
|---|---|
| Salesforce | `getDeleted()`（保留 30 天）、`queryAll` + `IsDeleted` |
| HubSpot | Webhook `contact.deletion`，或定期全量主键对账 |
| 无删除接口的 | 只能定期拉全量主键做集合差，**成本计入配额预算** |

**不能默默当作没发生**：silver 层记录消失必须留痕（软删除标记 + Ledger 一条），
否则下游看到的现象是「数据静默变少」，而这是最难排查的一类问题。

#### OAuth 的「只读」不等于 GRANT 的「只读」

第 8 节表格里「每个源一个专用账号，只授 `SELECT`」在 SaaS 上要打折扣——
**多数平台的 OAuth scope 粒度远不如数据库权限**：
...
**但「宽度」里有一类是例外：SaaS / API 源**（8.2）。它不是再写一个适配器，
而是要求 Connector Service 长出第二套准入与配额机制——SQL 那套在它上面一条都不成立。
因此它排在 R3，且**必须在 R4 的权限治理之前**：
...
| **R3** | 入站交互（IMAP 收信 / header 归属 / 意图分类 / 双重确认）+ 平台安全基线（TLS→认证→授权）+ AST 准入 + 数据源宽度（含 **SaaS 源**）+ 增量同步 | ⬜ | 匿名连不上 Trino；五种接入结果等价；关系展开被拒、配额预留生效；转发的链接批不动 |
...
| R3 | 「lakehouse 安全基线；五种异构接入路径结果等价；为 SaaS/API 源设计独立准入层——对象白名单文法拒绝关系展开、按官方配额 20% 做预留式限流、不可变主键分页 + 水位线回退解决增量一致性；增量同步与新鲜度 SLA」 |
...
**先做一个，做透；不做第二个。** 一个源足以验证 8.2 的全部机制，
第二个只是重复适配器工作，收益递减。

推荐 **Salesforce Developer Edition**（免费、永久、可申请示例数据），理由是它同时踩中四个点：

| 点 | 为什么是它 |
|---|---|
| 关系展开难挡 | SOQL 支持关系查询与子查询，**逼你写独立的白名单文法**，而不是复用 `sqlglot` 糊过去 |
| 配额小到会疼 | Developer Edition 每日 15,000 次调用，**预留机制真的会触发**，不是纸面设计 |
| 权限模型够真 | Profile / PermissionSet 是 R4 权限发现最好的素材，能跑出真实结论 |
| 只读要靠对方证明 | `api` scope 全有全无，正好演示 8.2 最后那条「留证据」的必要性 |

时间富余再加 **HubSpot**（免费开发者测试账号）。它的价值不在数据，
在于 **scope 粒度和 Salesforce 正相反**——两个一起才证明得了「适配器抽象是对的，
不是照着一家的形状写死的」。

面向中文场景的 Demo 可以换 **飞书多维表格**：口径确认那一幕更贴近国内公司，
但它的权限模型太浅，撑不起 R4，因此只作补充、不作主源。
...
2. **R3 的第二个 SaaS 源** —— 一个源已经验证完 8.2 的全部机制，第二个只是重复适配器
3. **R3 的 Excel / 邮件附件源** —— 保留 Postgres + CSV + 一个 SaaS 已足够证明「异构接入」
4. **R4 的 Policy Sync 自动下发** —— 可降级为「生成策略文件，人工 apply」
5. **R5 的用例数量** —— 从 300 降到 100，指标口径不变

**不能砍的是 SaaS 源本身。** 砍掉它，「20–50 人公司的数据散落在哪」这个前提就没被验证——
那个规模的公司数据大头在 CRM 和在线表格里，只演 Postgres + CSV 等于只演了最容易的一半。
...
| 17b | **SaaS 源准入与配额预留（对象白名单、禁关系展开、20% 配额）** | 8.2 | ❌ | **R3** |
| 17c | SaaS 侧权限现状发现 | 11.2 | ❌ | R4 |
| 35 | 增量同步与新鲜度 | 6.1 | ❌ | R3 |
