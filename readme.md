...
| **R3** | 平台安全基线（TLS→认证→授权）+ AST 准入 + 数据源宽度（CSV / Excel / 邮件附件）+ 增量同步 | ⬜ | 匿名连不上 Trino；四种接入结果等价 |
...
这一轮把散落在设计里的人机协作机制一次做完，因为它们彼此耦合，分开做反而不干净：

| 能力 | 章节 |
|---|---|
| 停止点判定、阶段成果交付 | 5.2 |
| **提问规范：八类必须问的、问题构造、答案沉淀** | **5.7** |
| DQ 门槛量化、3 轮修复上限、收敛判定 | 5.3 |
| 超时逐级升级 → ABANDONED、并行不阻塞 | 5.4 |
| 事件驱动 + 周报双轨汇报 | 5.5 |
| IMAP 收信、header 归属、意图分类、入站路由 | 10.1–10.3 |
| 角色化审批（`role_assignment`）、中途换人 | 10.4 |
| 双重确认（转发批不动）、回执 | 10.5–10.6 |
| WIP 限制（每人 3 件 / 全局 20 件） | 10.7 |

**R2 结束时，Demo 剧本里最有说服力的那几幕就齐了**：
转发的审批链接批不动、WIP 触顶转向、无人回信时优雅放弃、中途换人自动重发。
这几幕与用几种数据源、接了几张表**完全无关**。
...
| **R2** | 「设计并实现长时任务的挂起/恢复：run 状态落 event log，进程崩溃可续；超时逐级升级至优雅放弃，任务 DAG 并行不阻塞；WIP 限制防止淹没审批人」 ← **含金量最高** |
...
回归入口：`HERMES=<hermes-agent 路径> ./tests/run_all.sh` → **99 条断言全绿**
...
| 20 | **通知通道可替换（邮件 / 飞书 / 企微）** | 13.2 | ✅ | 三通道可实例化；邮件已实测发信 |
| 21 | 回执（批准后告知批了什么） | 10.6 | 🟡 | callback 已接入，待真实通道验证 |
| 22 | Cloudflare Tunnel 编排 | 13.2 | 🟡 | compose 已就绪，待填 token |
| 16 | EXPLAIN 预估、串行队列、熔断 | 8.1 | 🟡 | 接口在，护栏逻辑待补 → R3 |
| 23 | AST 准入（替换关键字判断） | 8.1 | 🟡 | 占位实现，`TODO(R3)` 换 sqlglot |
| 17 | 四种接入方式（SQL / CSV / Excel / 邮件附件） | 16.4 | ❌ | R3 |
| 24 | 增量同步与新鲜度 | 6.1 | ❌ | R3 |
| 25 | **平台安全基线（TLS / 认证 / 授权）** | 15 R3 | ❌ | **R3——卡住 R4 的权限治理** |
| 26 | 角色化审批、双重确认、回执 | 10.4–10.6 | ❌ | **R2** |
| 27 | IMAP 收信、意图分类、入站路由 | 10.1–10.3 | ❌ | **R2** |
| 28 | WIP 限制 | 10.7 | ❌ | **R2** |
| 29 | 停止点、DQ 门槛、3 轮上限、周报 | 5.2、5.3、5.5 | ❌ | **R2** |
| 30 | Remediation Ledger 与整改闭环 | 7、7.1 | ❌ | R4 |
| 31 | 权限治理与 Policy Sync | 11 | ❌ | R4 |
| 32 | Dirty Data Injector 与 Eval | 16 | ❌ | R5 |
| 33 | Phoenix 可观测 | 13 | ❌ | R1.5 |
