> 历史快照（2026-09-05 整理前）。包含旧设计、过时进度和已纠正的表述，
> 仅供追查旧章节与决策背景，不作为当前规范、实现状态或执行指令。
> 当前入口：[README](../../readme.md) · [CLAUDE.md](../../CLAUDE.md)。

# Data Steward Claw —— 项目约定

本项目的 agent 约定与开发规范统一在本文件读取和维护。新增、修改或删除约定时，
只更新 `CLAUDE.md`；`AGENTS.md` 仅保留入口指引，不维护重复规则。

## 全局偏好

- 跟随用户的输入语言回复：用户用中文就用简体中文，用纯英文就用纯英文回复。
- 默认用简体中文。不要因为上下文里出现其他语言（日语、瑞典语、英语的源数据或讨论）就把回复语言漂移走；回复语言只取决于用户当前消息用的语言。如果发现自己切换错了语言，立刻改回来。
- 回复默认只给结论/成品，不给过程、不给理由、不给备选方案、不做结尾总结段。用户追问“为什么/细节/展开”时再解释。不要用“很好的问题”“当然”这类开场。
- 用户说“我来讲一遍”后进入费曼模式：用户用自己的话复述，agent 只做三件事——指出讲错的、点出漏掉的关键、追问一个用户绕过去的地方。不要替用户补全，不要夸。
...
完整设计见 [readme.md](../../readme.md)，行业数据支撑见
[docs/industry-context.md](../../docs/industry-context.md)。本文件集中维护约定与开发规范；
变化的阶段进度写在 `docs/handoff/`。

开工前先读本文件，再读 [docs/handoff/](../../docs/handoff/) 中当前阶段的交接记录
（含「不要做的事」清单），以及 readme 第 19 节的实现状态表。
...
未声明的工具默认 **L4（拒绝）**，且不发审批。新增工具时必须在 `policy.py` 显式声明级别。
...
- Docker 数据盘在外置盘上；Mac 睡眠后若 `docker ps` 超时、daemon 连不上或 I/O error，
  先用 `diskutil list external physical` 和 `ls /Volumes` 确认外置盘挂载。
  若盘已挂载，下一次优先试验：
...
HERMES=<hermes-agent 路径> ./tests/run_all.sh
python3 ops/claw-status.py
```

测试环境、通过标准和跳过项见下方开发约定；不要在入口文件重复维护断言数。
