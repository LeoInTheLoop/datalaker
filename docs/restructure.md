...
### 6.5 M5 的设计：驱动反转要动的是「谁记账」

M5 一句话：`tests/run_case_full.py` 从**扮演 Agent** 改成**扮演人**。
脚本只做四件事 —— 发邮件、点审批链接、推时钟、读库判分；
一次也不许再调 `runs.RUNNERS["ingest_table"]`。

#### 挡在前面的那块石头：run 状态现在由脚本的胳膊记

今天这条链路是：

```
脚本 → runs.RUNNERS["ingest_table"]  →  Pipeline → gate（PENDING）
                     └── 回来之后由它调 runs.suspend / runs.finish
```

`runs.suspend` / `runs.finish` 全写在 `pipelines/ingest_table.py` 的
`as_run` 包装里。那个包装**就是脚本伸出去的胳膊**。
换成 Hermes 调工具之后：

```
Hermes → tool_call("ingest_table") → gate（PENDING，工具 handler 根本没跑）
                                          └── 没有任何人记这条线
```

于是「16 条线，等人 15」这类数字全部落空 —— 不是 Agent 没干，是没人记账。

**所以 M5 的第一步不是改脚本，是把记账挪到门禁一侧。**
`_gate` 里 `st.request(...)` 之后已经有一个 `_notify_async`（告诉人），
记账就挂在它旁边（告诉登记表）。两个都是**观察者**：不改变放行与否，
所以不违反铁律 1 —— 铁律 1 禁的是把**限制**散出去，不是禁止记录发生了什么。
...
|---|---|---|
| 谁决定调哪个工具 | 剧本（我写的） | 模型 |
| 能证明 | **管道通**：挂起 / 审批 / 恢复 / 入站 / 记账全程走 Hermes | **判断对**：该问的问了、该停的停了 |
| 不能证明 | 判断力 —— 桩里没有任何非确定性 | 可重复性 |

**两种都要有，且必须分开报数。** R4 已经踩过一次：加上 `LLM_PERSONA=1`
之后完成数从 15 掉到 3–6，说明在那之前「非确定性」下的数字是假的 ——
因为压根没有非确定性。桩模式的数字只能用来说「管道通」，
一旦拿它去说「Agent 判断对」，就是拿自己写的剧本给自己打分。

#### 顺带能删掉的

入站改走 Hermes 的 email adapter（见 6.4），`services/inbound.py` 里
那段过渡的发件人验真随之删掉。
