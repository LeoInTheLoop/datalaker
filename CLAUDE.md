...
| `docs/eval-model.md` | **两层测评共用的术语源**：Case / Snapshot / Trajectory / max_turn / Evaluator，含行为 eval 字段对应表 |
...
1. **约束由确定性代码执行。** Agent 工具授权统一走 `pre_tool_call`；
   ...
   **这一条的作用域是约束本身**：谁能做什么、SQL 能不能跑、审批算不算数。
   它不覆盖判断、检索与生成 —— 那三类的默认形态见下面「形态先定」。
...
**动手前先声明这次的形态**：哪一段是确定性代码，哪一段交给模型，为什么。
一句话写进回复或提交信息即可，不用写文档 —— 要的是这个判断真的发生过。

默认分工：

| 这件事是 | 默认形态 | 为什么 |
|---|---|---|
| 约束、授权、审计轨、判分 | **确定性代码** | 见执行边界 1。判分器还须能离线跑，不依赖模型端点 |
| 判断（这两列是不是同一个实体、这张表归谁、这条口径还成不成立） | **模型，带证据** | 需要业务语义。启发式在跨语言、跨命名习惯上必然漏 |
| 检索（该把什么放进上下文、放多少） | **模型按需取** | 固定模板要么塞爆上下文，要么漏掉这次真正要的那条 |
| 生成（口径、提案、摘要） | **模型** | —— |

**把判断写成确定性启发式要写明理由**（成本、可复现、判分器不能依赖模型），
并说明它漏掉的是哪一类输入。没写理由的阈值和白名单按跑偏处理。
反过来，**把约束交给模型**一律不行，无论理由多充分。

启发式与模型可以叠：便宜的确定性打分做**粗筛**（省钱、省调用），
模型在粗筛结果上做**判定**。粗筛的阈值定成宁可多给几条，
不要定成「这条我觉得不像」—— 那已经是判定了。
巡检类作业沿用同一形状：脚本先跑、内容没变就不点模型。

给现有项目加记忆、上下文、工具权限、可观测性这类模块时，
先走 `agent-patterns` skill 的 apply 模式定编排形态，再动手。

## 测评链路

概念定义、Calibration、pass^k 与不泄题红线在 [docs/eval-model.md](docs/eval-model.md)，
改测评结构前先读那份。开发时必须守住的是下面五条：

- **Case 定义世界，Snapshot 是某时刻可恢复的完整状态。** 加场景默认加 Snapshot；
  换人、换数据资产、换权限边界才需要新 Case。表清单属于 Snapshot，不属于 Case。
- **前置条件注入 state，不靠历史消息表达。** 默认形状是 **state + 一条当前消息**。
  「已连接、已登记、已批准」这些直接写成库里的事实，不要去构造一串邮件往来把它
  「演」出来 —— 那串往来不是还原，是另一次测试。历史消息只在**被测行为本身就是
  跨 turn 带状态**时才放（例外：只在很长很乱很矛盾的历史之后才出现的缺陷，那种
  历史就是被测条件）。Old Trajectory 只进页面和 audit，不重新塞给模型。
  **把过去总结成一段话喂进去，测出来的就不是原来的系统。**
- **判断一条信息能不能进模型上下文，只问一句**：生产环境里的 Hermes 在那一刻，
  是不是真的就看到这个东西？真实收到过的邮件是，进；演练元数据
  （`note` / `table_scope` / `terminal_condition` / `tool_scope` / Case 与 Snapshot
  标题）都不是，不进。要它知道的环境事实，让它**查得到**，不是在正文里读到。
- **Gate 定边界，max_turn 定窗口，两者都不是答案。** 目标达成即止和没达成也要停是
  两层，都得有：前者在 cron 唤醒侧认真实成功事件，后者在 `pre_tool_call` 数本轮
  turn。一次 turn = 一次进 gate 的工具调用，被 block 的也算。
- **判分优先看环境结果**，New Trajectory 用来理解为什么。路径判据只用在路径本身
  就是被测行为的地方；**实测走了意料外的路但结果对，是放宽 case，不是把 case 钉到
  刚观测到的那条路上**。Agent 是随机系统，一个 Snapshot 要跑多次看 pass^k，
  比的是失败集合不是总分；一次通过不算结论。
