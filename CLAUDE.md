...
**SaaS / API 源同理，但机制要单独写一套**（readme 8.2），不要试图复用 SQL 那套：

- **单对象读取**，禁止一切关系展开：SOQL 关系查询与子查询、HubSpot `associations`、
  Stripe/Notion `expand` —— 这是「永不 join」的 API 对应物，理由逐字相同
- SOQL **不能用 `sqlglot` 解析**（不是 SQL 方言，会得到似是而非的树）。
  写独立的**白名单文法**准入，宁可拒错也不放行没见过的写法
- **配额先预留再调用**：API 没有 `EXPLAIN`，翻到最后一页才知道有多少。
  每源每日上限默认取官方配额的 **20%**，其余留给对方的生产集成
