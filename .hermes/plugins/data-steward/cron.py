...
JOBS = {
    "data-steward-resume": {
        ...
        "prompt": (
            ...
            "同样照 `args` 直接重试。\n"
            "`kind=\"due\"`：等的是**某个时刻**，那个时刻到了（例如已批准、"
            "刚进入执行窗口）。照 `args` 调 `tool`；**到点只说明可以开始看了**，"
            "不代表条件都还成立 —— 被门禁拦下就说明还缺什么，跳过它继续下一条。\n"
            "`kind=\"silver_ready\"`：不是等人的线，是**清洗轮已经开了、"
            "这张表还没洗**。先 `propose_cleaning` 看提案，"
            "再用提案里给出的规则名（原样抄，别改写）调 `apply_cleaning_rule`。\n"
            "`kind=\"stop_point\"` / `kind=\"blocker\"`：**这不是让你继续干活，"
            "是让你停下来交成果。** 行里写着 deliverable（要交什么）和 "
            "question（要问什么）：把已完成的部分整理成那份交付物，"
            "然后把那个问题发给能拍板的人，发完就停。不要替人做决定，"
            "也不要因为「差不多能猜到答案」就跳过这一步继续往下做。\n"
